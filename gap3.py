...
'''
The traceback indicates a KeyError: 'open' in the generate_signal method of the GapTradingStrategy class. This means that when the code tries to access the 'open' column of the self.data DataFrame using self.data['open'].iloc[current_index], the 'open' column is not found. The KeyboardInterrupt is secondary - it was probably caused by you interrupting the long run. Let's address the primary error first.

Here's a breakdown of why this likely happened and how to fix it:

Data Integrity After Historical Fetch and Preprocessing: The most likely cause is that the open column was dropped or renamed during the historical data fetching or preprocessing steps. Even though you fetch the data with 'open', 'high', 'low', 'close', and 'volume', the preprocessing steps could be unintentionally modifying the DataFrame in a way that removes the 'open' column.

Index Mismatch: It's less likely in this case, because the error is a KeyError and not an IndexError. However, if the index of the DataFrame gets messed up during preprocessing, it could indirectly lead to problems with accessing data by integer location (iloc).

Empty DataFrame (though you have a check): While you do have a check for empty historical data, it's worth verifying that the preprocessing steps aren't somehow resulting in an empty DataFrame after the initial data fetch.

Here's the corrected code with debugging and safety checks:
...
def handle_missing_values(df, columns, method='ffill'):
    ...
    for col in columns:
        if col not in df.columns:
            print(f"Warning: Column '{col}' not found in DataFrame for missing value handling.")  # ADDED
...
def handle_outliers_iqr(df, columns, multiplier=1.5):
    ...
    for col in columns:
        if col not in df.columns:
            print(f"Warning: Column '{col}' not found in DataFrame for outlier handling.") # ADDED
            continue # Skip this column
...
class GapTradingStrategy:
    ...
    def generate_signal(self, current_index: int) -> str:
        ...
        try:
            # Check if 'open' column exists before accessing
            if 'open' not in self.data.columns: # ADDED CHECK
                print("Error: 'open' column missing in DataFrame at generate_signal.")
                return "HOLD"  # Or raise an exception, depending on how you want to handle this

            # Check if 'close' column exists before accessing
            if 'close' not in self.data.columns: # ADDED CHECK
                print("Error: 'close' column missing in DataFrame at generate_signal.")
                return "HOLD"  # Or raise an exception, depending on how you want to handle this
...
class Backtester:
    ...
    def run_backtest(self):
        ...
        for i in range(len(self.data)):
            ...
            if signal == "BUY":
                if self.current_position_shares == 0: # Only buy if not already in a position
                    ...
                    shares_to_buy = shares_to_buy * (1 - self.commission_rate) # Account for commission on purchase
                    self.current_position_shares = shares_to_buy
                    self.entry_price = actual_entry_price
                    self.entry_time = current_timestamp
                    self.cash -= shares_to_buy * actual_entry_price # Deduct actual cost
                ...
                if self.current_position_shares > 0: # Only sell if in a long position
                    ...
                    self.cash += self.current_position_shares * actual_exit_price
                    self.cash = self.cash * (1 - self.commission_rate) # Account for commission on sale
        ...
        if self.current_position_shares > 0:
            ...
            profit_loss = (actual_exit_price - self.entry_price) * self.current_position_shares
            self.cash += self.current_position_shares * actual_exit_price
            self.cash = self.cash * (1 - self.commission_rate)
            self.trades.append({
                'type': 'FINAL_SELL',
                'entry_price': self.entry_price,
                'exit_price': actual_exit_price,
...
async def main():
    ...
    # DEBUG: Print column names after fetching data
    print("Columns after fetching historical data:", initial_data.columns)
    ...
    print("Columns after preprocessing data:", data_for_strategies.columns)
