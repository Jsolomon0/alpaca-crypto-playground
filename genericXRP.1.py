...
class GenericStrategy:
    def __init__(self, data):
        """
        Initializes the Generic Strategy with historical data.
        :param data: pandas DataFrame containing historical price data.
        """
        self.data = data
        self.data['rsi'] = self.calculate_rsi(window=14)
        self.data['sma_50'] = self.data['close'].rolling(window=1440).mean()
        self.data['momentum'] = self.data['close'] - self.data['sma_50']

    def calculate_rsi(self, window=15):
    ...
    def generate_signal(self, index):
        """Generates a trading signal based on RSI and Momentum."""
        if index < 1440:  # Need at least 50 periods for indicators
            return "HOLD"

        if self.data['rsi'].iloc[index] < 30 and self.data['momentum'].iloc[index] > 0:
            return "BUY"
        elif self.data['rsi'].iloc[index] > 70 or self.data['momentum'].iloc[index] < 0:
            return "SELL"
        else:
            return "HOLD"
